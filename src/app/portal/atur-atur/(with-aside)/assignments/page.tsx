...
export default async function Assignments() {
  ...
  const classes = await Promise.all(courses.map(async ({id, title}) => {
    const classes_ = await getClasses(id);
    return classes_.map((class_) => ({courseId: id, courseTitle: title, class: class_}));
  }));

  const assignments_ = await Promise.all(classes.map(async (c_) => {
    return await Promise.all(c_.map(async (c) => {
      const as = await getAssignments(c.courseId, c.class.id);
      return as.map((a) => ({...a, course: c.courseTitle, class: c.class}));
    }))
  }));
